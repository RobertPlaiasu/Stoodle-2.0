...
@section('title', 'Editare universitate')
...
@section('content')
    <div class="form">
        <div class="container">
            ...
            <form action="{{ route('university.update', $university->id) }}" method="POST" id="formular" enctype="multipart/form-data">
                @method("PUT")
                @csrf
                ...
                <input type="text" id="name" name="name" class="form-control" value="{{ $university->name }}" placeholder="Numele universitatii">
                    @error('name')
                        {{ $message }}
                    @enderror
                </div>
